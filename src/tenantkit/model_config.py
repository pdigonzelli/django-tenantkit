"""
Model configuration system for django-tenantkit.
...
Usage:
    ...
    from tenantkit.model_config import SharedModel, TenantModel
    ...
    from tenantkit.model_config import shared_model, tenant_model
...
class ModelRegistry:
    ...
    def get_model_config(
        cls, model_class: type[models.Model] | str
    ) -> dict[str, Any] | None:
        """
...
def get_models_for_migration(model_type: str | None = None) -> list[type[models.Model]]:
    ...
    return [
        config["model_class"] for config in configs if config.get("auto_migrate", True)
    ]
